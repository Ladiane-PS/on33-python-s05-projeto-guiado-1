...
>>>>>>> 103313e7447e4388e2c4beaec711e3371ec907bb
...
<<<<<<< HEAD
#     pr.imprimir('[H]   >> Ajuda com o Sistema', alinhar='comeco', tamanho=120)  # Imprimimos a opção de ajuda '[H]'
...
<<<<<<< HEAD
    pr.imprimir(' compras  * ', 'compras * ', caracter='═', tamanho=115, alinhar='fim', end='👍    ')  # Imprimimos as opções de ajuda e sair no rodapé
=======
...
<<<<<<< HEAD
    pr.imprimir(' compras  * ', 'compras * ', caracter='═', tamanho=115, alinhar='fim', end='👍   ') 
=======
...
def calcula_total_desconto(compra):  # Esta função recebe a compra como entrada
    ...
    total = 0
    itens_por_codigo = {} # diconario para armazenar na lista 
    # Percorremos a lista de produtos
    for produto in compra:  # Para cada produto na lista de produtos
        codigo = produto['codigo'] #tipo do produto
        quantidade = produto['quantidade']
        valor = produto['valor']

        #verifiocar se o tipo do produto já está no dicionário
        if codigo not in itens_por_codigo:
            itens_por_codigo[codigo] = 0 # inicializamos a quantidade do tipo do produto

        #calculand o total do produto para cada regra do combo    
        for i in range(quantidade):
            if itens_por_codigo[codigo] >= 1: #se já comprou pelo menos um item desse tipo, aplicar o descontod e 50%
               total += valor *0.5
            else: # se é o primeiro item desse tipo, não aplicar o desnconto
                total_por_codigo[codigo] += 1 # incrementar a quantidade do tipo de produto no dicioáro   
